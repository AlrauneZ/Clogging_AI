...
for ip,param in enumerate(params):
    ...
    for i in range(3):
        ...
        ax.plot(x,score,marker = 'o',ls = '-',lw=1,markersize = 2)
...
plt.tight_layout()
fig.legend(val_hyper[0:len(i_hyper)], ncol = len(i_hyper),bbox_to_anchor=[0.75, 0.06],fontsize=textsize)#,&nbsp)
fig.subplots_adjust(bottom=0.13)# Adjusting the sub-plots
fig.text(0.02,0.02,'{}orable conditions'.format(condition),fontsize=textsize, bbox=dict(facecolor='w', alpha=0.2,boxstyle='round'))#transform=ax1.transAxes)

# plt.savefig('../results/Fig_RBFNN_{}.png'.format(condition),dpi=300)   
plt.savefig('../results/FigS03_RBFNN_Hyper_MSE_{}.pdf'.format(condition))   
