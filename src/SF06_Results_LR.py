...
for ic, cond in enumerate(conditions):
    ...
    for ip,param in enumerate(params):
        ...
        for it,n_it in enumerate(iterations):
            data = np.loadtxt('../results/SI/LR_Training_{}_It{}.csv'.format(cond,n_it),delimiter = ',',skiprows=1)
            ax.plot(data[:,0],data[:,1+ip],marker = 'o',ls = '-',lw=2,markersize = 4)

        data = np.loadtxt('../results/SI/LR_Training_{}_full.csv'.format(cond),delimiter = ',')
        ax.plot(data[:,0],data[:,1+ip],marker = 'o',ls = '-',lw=2,markersize = 4)
...
plt.savefig('../results/FigS06_LR_Hyper.pdf')   
