...
for ic, cond in enumerate(conditions):
    ...
    for it,n_it in enumerate(iterations):
        data = np.loadtxt('../results/LR_Training_{}_It{}.csv'.format(cond,n_it),delimiter = ',',skiprows=1)
    ...
    for ip,param in enumerate(params):
        ...
        data = np.loadtxt('../results/LR_Training_{}_full.csv'.format(cond),delimiter = ',')
        ax.plot(data[:,0],data[:,1+ip],marker = 'o',ls = '-',lw=2,markersize = 4)
        ...
        if ic == 0:
            ...
            ax.set_xlabel(r'$\alpha$',fontsize=textsize)
            if ip == 0:        
                ax.set_ylabel('$R^2$ - unfav',fontsize=textsize)

plt.tight_layout()
fig.legend(iterations+['full'], ncol = 7,bbox_to_anchor=[0.850, 0.105],fontsize=textsize)#,&nbsp)
fig.subplots_adjust(bottom=0.24)# Adjusting the sub-plots

plt.savefig('../results/FigS06_LR_Hyper_new_Py.pdf')   
